...
def run_kinetics(file_name, ex_VE_df):
    ...
    last_60s_rest_df = ex_VE_df[(ex_VE_df['ExTime'] >= -60) & (ex_VE_df['ExTime'] <= 0)]
    A0_mean = last_60s_rest_df['VE'].mean()
    print(f"A0_mean{A0_mean}")
    
    # Filter only positive VE values
    ex_VE_df_positive = ex_VE_df[ex_VE_df['ExTime'] >= 0]
    if ex_VE_df_positive.empty:
        print(f"No positive VE values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {ex_VE_df.shape[0] - ex_VE_df_positive.shape[0]} negative exercise time values.")
        print(f"{ex_VE_df_positive.shape[0]} positive exercise time values.")

    # Extract time and VE (all positive exercise values from here)
    x_data = np.array(ex_VE_df_positive['ExTime'])
    y_data = np.array(ex_VE_df_positive['VE'])
    ...
    area_under_curve = (quad(fitted_VE, x_start, x_end)[0]) / 60 # Perform definite integral for the final model results and convert it to liters  
    total_area = (A1 / 60) * x_end # End result to L
    print(f"Total exercise area = {total_area}") 
    print(f"Area under curve = {area_under_curve}")
    VE_def = total_area - area_under_curve
    
    # Poole & Jones 2012 estimate
    VE_on_PC = (A1-A0)
    VE_def_estimate = (tau/60) * VE_on_PC
    VE_MRT = TD + tau
    ...
    print(f"VE deficit = {VE_def:.4f}")
    print(f"VE_def_estimate = {VE_def_estimate:.4f}")
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_VE_on_residuals.png")
    ...
    plt.ylabel("VE (L/min)")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_VE_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "VE_on_A0": A0,# L/min
        "VE_on_A1": A1,# L/min
        "VE_on_PCA": VE_on_PC, # L/min
        "VE_on_TD": TD, # sec
        "VE_on_tau": tau, # sec
        "VE_on_MRT": VE_MRT, # sec
        "VE_on_Final_R^2":r_squared,
        "VE_on_RMSE":rmse, # L/min
        "VE_on_CI_A0": ci[0], # L/min
        "VE_on_CI_A1": ci[1], # L/min
        "VE_on_CI_TD": ci[2], # sec
        "VE_on_CI_tau": ci[3], # sec
        "VE_on_VE_def": VE_def, # Liters
        "VE_on_VE_def_estimate": VE_def_estimate, # Liters
        "Total_VE_liters":total_area,
        "VE_on_TT_ss": TT_ss # sec

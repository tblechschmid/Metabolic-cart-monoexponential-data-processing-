...
# Main function to run kinetics model for Matched HR
def run_kinetics(file_name, ex_VO2_df):
    ...
    A0_mean = last_60s_rest_df['Matched HR'].mean()
    print(f"A0_mean{A0_mean}")
    
    # Filter only positive Matched HR values
    ex_Matched_HR_df_positive = ex_VO2_df[ex_VO2_df['ExTime'] >= 0]
    if ex_Matched_HR_df_positive.empty:
        print(f"No positive Matched HR values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {ex_VO2_df.shape[0] - ex_Matched_HR_df_positive.shape[0]} negative exercise time values.")
        print(f"{ex_Matched_HR_df_positive.shape[0]} positive exercise time values.")

    # Extract time and Matched HR (all positive exercise values from here)
    x_data = np.array(ex_Matched_HR_df_positive['ExTime'])
    y_data = np.array(ex_Matched_HR_df_positive['Matched HR'])
    ...
    for iteration in range(5):
        ...
        # Prepare TD_initial guess (First breath where Matched HR experiences a +5% change and is sustained for 15 seconds)
        upper_threshold =  A0_mean* 1.10
        # Time and Matched HR arrays
        time_vals = x_data
        HR_vals = y_data
        # Create a boolean mask where HR > 105% of A0_mean
        ...
        A1_lb = A0_mean  #Contrains HR ot resting HR
    ...
    area_under_curve = (quad(fitted_HR, x_start, x_end)[0]) / 60 # Perform definite integral for the final model results and convert it to HR to breaths 
    ...
    HR_def = total_area - area_under_curve
    
    # Poole & Jones 2012 estimate
    HR_on_PCA = (A1-A0)
    HR_def_estimate = (tau/60) * HR_on_PCA
    HR_on_MRT = TD + tau
    ...
    print(f"HR deficit = {HR_def:.4f}")
    print(f"HR_def_estimate = {HR_def_estimate:.4f}")
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_HR_on_residuals.png")
    ...
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_HR_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "file": file_name,
        "HR_on_A0": A0, # beats/min
        "HR_on_A1": A1, # beats/min
        "HR_on_PCA": HR_on_PCA, # beats/min
        "HR_on_TD": TD, # sec
        "HR_on_tau": tau, # sec
        "HR_on_MRT":  HR_on_MRT, # sec
        "HR_on_RMSE":rmse, # beats/min
        "HR_Final_R^2":r_squared,
        "HR_on_CI_A0": ci[0], # beats/min
        "HR_on_CI_A1": ci[1], # beats/min
        "HR_on_CI_TD": ci[2], # sec
        "HR_on_CI_tau": ci[3], # sec
        "HR_on_HR_def": HR_def, # beats
        "HR_on_HR_def_estimate": HR_def_estimate, # beats
        "Total_HR_beats":total_area, # beats
        "HR_on_TT_ss": TT_ss # sec

...
def run_kinetics(file_name, rec_VO2_df):
    ...
    y_data = np.array(rec_VO2_df_positive.iloc[:, 1])
    ...
    for iteration in range(5):
        ...
        time_vals = x_data
        vo2_vals = y_data
        # Create a boolean mask where VO2 < 95% of A0_mean
        ...
        if not found:
            print(f"No sustained -5% VO2 change found; fallback to 10 s")
        
        #Set lower bounds for the equation
        A0_lb = A0_init - 1e-3  # constrains the lower bound to very close to A0_mean
        A1_lb = y_data.min()  #Contrains lowest VO2 observed in recovery 
        ...
        td_ub = x_data.max() # No change in VO2 from end exercise 
    ...
    area_under_curve = (quad(fitted_vo2, x_start, x_end)[0]) / 60 # Perform definite integregral for the final model results and convert it to liters  
    rest_VO2_area = (A1 / 60) * x_end # dividing by 60 converts the result to L
    print(f"Rest_VO2_area = {rest_VO2_area}") 
    print(f"Area under curve = {area_under_curve}")
    EPOC = area_under_curve - rest_VO2_area
    
    # Poole & Jones 2012 estimate
    VO2_off_pc = (A0- A1)
    EPOC_estimate = (tau/60) * VO2_off_pc
    VO2_MRT = TD + tau
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_VO2_off_residuals.png")
    ...
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_VO2_off_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "VO2_off_A0": A0, # L/min
        "VO2_off_A1": A1, # L/min
        "VO2_off_PCA": VO2_off_pc, # L/min
        "VO2_off_TD": TD, # sec
        "VO2_off_tau": tau, # sec
        "VO2_off_MRT": VO2_MRT, # sec
        "VO2_off_Final_R^2":r_squared,
        "VO2_off_RMSE":rmse, # L/min
        "VO2_off_CI_A0": ci[0], # L/min
        "VO2_off_CI_A1": ci[1], # L/min
        "VO2_off_CI_TD": ci[2], # sec
        "VO2_off_CI_tau": ci[3], # sec
        "VO2_off_EPOC": EPOC, # Liters
        "VO2_off_EPOC_estimate": EPOC_estimate, # Liters
        "VO2_off_TT_ss": TT_ss # sec

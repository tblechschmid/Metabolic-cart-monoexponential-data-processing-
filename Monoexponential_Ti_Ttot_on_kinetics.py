...
# Main function to run kinetics model for Ti_Ttot
def run_kinetics(file_name, ex_Ti_Ttot_df):
    ...
    last_60s_rest_df = ex_Ti_Ttot_df[(ex_Ti_Ttot_df['ExTime'] >= -60) & (ex_Ti_Ttot_df['ExTime'] <= 0)]
    A0_mean = last_60s_rest_df['Ti_Ttot'].mean()
    print(f"A0_mean{A0_mean}")
    
    # Filter only positive Ti_Ttot values
    ex_Ti_Ttot_df_positive = ex_Ti_Ttot_df[ex_Ti_Ttot_df['ExTime'] >= 0]
    if ex_Ti_Ttot_df_positive.empty:
        print(f"No positive Ti_Ttot values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {ex_Ti_Ttot_df.shape[0] - ex_Ti_Ttot_df_positive.shape[0]} negative exercise time values.")
        print(f"{ex_Ti_Ttot_df_positive.shape[0]} positive exercise time values.")

    # Extract time and Ti_Ttot (all positive exercise values from here)
    x_data = np.array(ex_Ti_Ttot_df_positive['ExTime'])
    y_data = np.array(ex_Ti_Ttot_df_positive['Ti_Ttot'])
    ...
    for iteration in range(5):
        ...
        # Prepare TD_initial guess (First breath where Ti_Ttot experiences a +5% change and is sustained for 15 seconds)
        upper_threshold =  A0_mean* 1.10
        # Time and Ti_Ttot arrays
        time_vals = x_data
        Ti_Ttot_vals = y_data
        # Create a boolean mask where Ti_Ttot > 105% of A0_mean
        above_threshold = Ti_Ttot_vals > upper_threshold
        # Minimum sustained duration requirement
        min_duration = 15  # seconds
        # Default time delay value
        td_init = 80.2
        found = False  # Flag to check if a valid TD was found
        # Search for the first time Ti_Ttot stays above threshold for >= 30 seconds
        ...
        if not found:
            print(f"No sustained +5% Ti_Ttot change found; fallback to 10 s")
        
        #Set lower bounds for the equation
        A0_lb = A0_init - 1e-3  # constrains the lower bound to very close to A0_mean
        A1_lb = y_data.min()  #Contrains VO2 ot resting VO2
        td_lb = 0  
        tau_lb = 0  
        ...
        td_ub = x_data.max() # Indicates no change in variable of choice
    ...
    # Define the fitted Ti_Ttot function for integration
    def fitted_Ti_Ttot(t):
        return A0 + (A1 - A0) * (1 - np.exp(-(t - TD) / tau)) if t >= TD else A0
    # Integration bounds
    x_start = 0
    x_end = x_data.max()  # final x value converted to minutes
    area_under_curve = (quad(fitted_Ti_Ttot, x_start, x_end)[0]) / 60 # Perform definite integral for the final model results and convert it to liters  
    total_area = (A1 / 60) * x_end # End result to L
    print(f"Total exercise area = {total_area}") 
    print(f"Area under curve = {area_under_curve}")
    Ti_Ttot_def = total_area - area_under_curve
    
    # Poole & Jones 2012 estimate
    Ti_Ttot_on_PC = (A1-A0)
    Ti_Ttot_def_estimate = (tau/60) * Ti_Ttot_on_PC
    Ti_Tot_on_MRT = TD + tau
    ...
    print(f"Ti_Ttot deficit = {Ti_Ttot_def:.4f}")
    print(f"Ti_Ttot_def_estimate = {Ti_Ttot_def_estimate:.4f}")
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_Ti_Ttot_on_residuals.png")
    plt.savefig(residuals_plot_path)
    plt.show()
    plt.close()

    # Final Ti_Ttot model plot
    plt.figure(figsize=(10, 6))
    plt.scatter(x_data, y_data, label="Observed Ti_Ttot", color='blue', s=20)
    ...
    plt.title(f"Ti_Ttot Kinetics - {file_name}")
    plt.xlabel("Time (s)")
    plt.ylabel("Ti_Ttot")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_Ti_Ttot_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "file": file_name,
        # "Ti_Ttot_on_A0_init":A0_init,
        # "Ti_Ttot_on_A1_init":A1_init,
        "Ti_Ttot_on_A0": A0, 
        "Ti_Ttot_on_A1": A1,
        "Ti_Ttot_on_PCA": Ti_Ttot_on_PC, 
        "Ti_Ttot_on_TD": TD, #  sec
        "Ti_Ttot_on_tau": tau, # sec
        "Ti_Tot_on_MRT" : Ti_Tot_on_MRT, # sec
        "Ti_Ttot_RMSE":rmse, 
        "Ti_Ttot_Final_R^2":r_squared,
        "Ti_Ttot_on_CI_A0": ci[0],
        "Ti_Ttot_on_CI_A1": ci[1],
        "Ti_Ttot_on_CI_TD": ci[2],  # sec
        "Ti_Ttot_on_CI_tau": ci[3],  # sec
        "Ti_Ttot_on_def": Ti_Ttot_def, # Liters
        "Ti_Ttot_on_def_estimate": Ti_Ttot_def_estimate, # Liters
        "Ti_Ttot_on_TT_ss": TT_ss # sec

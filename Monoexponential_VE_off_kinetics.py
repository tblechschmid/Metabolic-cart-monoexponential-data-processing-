...
# Main function to run kinetics model for VE
def run_kinetics(file_name, rec_VE_df):
    ...
    last_60s_ex_df = rec_VE_df[(rec_VE_df.iloc[:, 0] >= -60) & (rec_VE_df.iloc[:, 0] <= 0)]
    A0_mean = last_60s_ex_df.iloc[:, 1].mean()
    print(f"A0_mean = {A0_mean}")
    
    # Filter only positive VE values
    rec_VE_df_positive = rec_VE_df[rec_VE_df['ExTime'] >= 0]
    if rec_VE_df_positive.empty:
        print(f"No positive VE values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {rec_VE_df.shape[0] - rec_VE_df_positive.shape[0]} negative exercise time values.")
        print(f"{rec_VE_df_positive.shape[0]} positive exercise time values.")

    # Extract time and VE (all positive exercise values from here)
    x_data = np.array(rec_VE_df_positive.iloc[:, 0])
    y_data = np.array(rec_VE_df_positive.iloc[:, 1])
    ...
    for iteration in range(5):
        ...
        # Prepare TD_initial guess (First breath where VE experiences a -5% change and is sustained for 15 seconds)
        upper_threshold =  A0_mean* 0.95
        # Time and VE arrays
        time_vals = x_data
        VE_vals = y_data
        # Create a boolean mask where VE > 105% of A0_mean
        above_threshold = VE_vals < upper_threshold
        # Minimum sustained duration requirement
        min_duration = 15  # seconds
        # Default time delay value
        td_init = 25.3
        found = False  # Flag to check if a valid TD was found
        # Search for the first time VE stays above threshold for >= 30 seconds
        ...
        if not found:
            print(f"No sustained -5% VE change found; fallback to 28.3 s")
        
        #Set lower bounds for the equation
        A0_lb = A0_init - 1e-3  # constrains the lower bound to very close to A0_mean
        A1_lb = y_data.min()  #Contrains lowest VE observed in recovery 
        td_lb = 0
        tau_lb = 0
    ...
    # Define the fitted VE function for integration
    def fitted_VE(t):
        return model(t, A0, A1, TD, tau)
    # Integration bounds
    x_start = 0
    x_end = x_data.max() #final x value
    area_under_curve = (quad(fitted_VE, x_start, x_end)[0]) / 60 # Perform definite integregral for the final model results and convert it to liters  
    rest_VE_area = (A1 / 60) * x_end # dividing by 60 converts the result to L
    print(f"Rest_VE_area = {rest_VE_area}") 
    print(f"Area under curve = {area_under_curve}")
    EPVE = area_under_curve - rest_VE_area
    
    # Poole & Jones 2012 estimate
    VE_off_pc = (A0- A1)
    EPVE_estimate = (tau/60) * VE_off_pc
    VE_MRT = TD + tau
    ...
    print(f"EPVE = {EPVE:.4f}")
    print(f"EPVE_estimate = {EPVE_estimate:.4f}")
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_VE_off_residuals.png")
    ...
    plt.scatter(x_data, y_data, label="Observed VE", color='blue', s=20)
    ...
    equation_text = f"VE(t) = {A0:.2f} - ({A1:.2f} - {A0:.2f}) × [1 - exp(-(t - {TD:.2f}) / {tau:.2f})]"
    plt.text(
    ...
    plt.title(f"VE Kinetics - {file_name}")
    plt.xlabel("Time (s)")
    plt.ylabel("VE (L/min)")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_VE_off_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "VE_off_A0": A0, # L/min
        "VE_off_A1": A1, # L/min
        "VE_off_PCA": VE_off_pc, # L/min
        "VE_off_TD": TD, # sec
        "VE_off_tau": tau, # sec
        "VE_off_MRT": VE_MRT, # sec
        "VE_off_Final_R^2":r_squared,
        "VE_off_RMSE":rmse, # L/min
        "VE_off_CI_A0": ci[0], # L/min
        "VE_off_CI_A1": ci[1], # L/min
        "VE_off_CI_TD": ci[2],  # sec
        "VE_off_CI_tau": ci[3], # sec
        "VE_off_EP_VE": EPVE, # Liters
        "VE_off_EP_VE_estimate": EPVE_estimate, # Liters
        "VE_off_TT_ss": TT_ss # sec

...
1.	Save your data from the Vyaire Vmax cart in .xlxs format.
  ...
  c. For participants with multiple tests on a single day, files should be saved with the extensions "_V1_T1" for treadmill test #1 at baseline, "_V1_T2" for treadmill test #2 at baseline, etc.
    i.	If you have called the visit variables differently than the _V1_T1 format for example, you will need to change the script (or your saving convention) to match your naming convention or the variable to match the script.  
3.	Place your .xlxs data files in the “data” subfolder within the working directory. 
4.	(ONLY IF NEEDED) Edit “File_processing” to pull data from the columns that you want to run. 
...
5.	(If no previsously published data for your protocol is avaiable) Run the “Average-all-indices-initial-guesses-and-bounds.py” script. to get the estimates for initial guesses of each measured parameter.
...
6.	(ONLY IF NEEDED) Edit “Batch_process” by commenting in/out the scripts that you want to run.
  a.	This will then only run the scripts in the folder that you need and shorten analysis time. 
7.	Make sure all the files that python will be running the script on are closed.
...
8.	Run batch process by clicking the play button or pressing f5 (Varies by IDE).
9.	When batch_process is done, open the “Kinetics_summary.xlsx” file and view the results.  

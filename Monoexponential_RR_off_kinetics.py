...
def run_kinetics(file_name, rec_VO2_df):
    ...
    A0_mean = last_60s_ex_df['RR'].mean()
    ...
    y_data = np.array(rec_VO2_df_positive['RR'])
    ...
    for iteration in range(5):
        ...
        time_vals = x_data
        RR_vals = y_data
        # Create a boolean mask where RR < 95% of A0_mean
        ...
        if not found:
            print(f"No sustained -5% RR change found; fallback")
        
        #Set lower bounds for the equation
        A0_lb = A0_init - 1e-3  # constrains the lower bound to very close to A0_mean
        A1_lb = y_data.min()  #Contrains lowest RR observed in recovery 
        ...
        td_ub = x_data.max() # No change in RR from end exercise 
    ...
    area_under_curve = (quad(fitted_RR, x_start, x_end)[0]) / 60 # Perform definite integregral for the final model results and convert it to liters  
    rest_RR_area = (A1 / 60) * x_end # dividing by 60 converts the result to L
    print(f"Rest_RR_area = {rest_RR_area}") 
    print(f"Area under curve = {area_under_curve}")
    EPOC = area_under_curve - rest_RR_area
    
    # Poole & Jones 2012 estimate
    RR_off_pc = (A0- A1)
    EPOC_estimate = (tau/60) * RR_off_pc
    RR_MRT = TD + tau
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_RR_off_residuals.png")
    ...
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_RR_off_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "RR_off_A0": A0,
        "RR_off_A1": A1,
        "RR_off_PCA": RR_off_pc, 
        "RR_off_TD": TD,
        "RR_off_tau": tau,
        "RR_off_MRT" : RR_MRT,
        "RR_off_Final_R^2":r_squared,
        "RR_off_RMSE":rmse,
        "RR_off_CI_A0": ci[0],
        "RR_off_CI_A1": ci[1],
        "RR_off_CI_TD": ci[2],
        "RR_off_CI_tau": ci[3],
        "RR_off_EP_RR": EPOC,
        "RR_off_EP_RR_estimate": EPOC_estimate,
        "RR_off_TT_ss": TT_ss

...
def run_kinetics(file_name, rec_Vt_df):
    ...
    last_60s_ex_df = rec_Vt_df[(rec_Vt_df.iloc[:, 0] >= -60) & (rec_Vt_df.iloc[:, 0] <= 0)]
    A0_mean = last_60s_ex_df.iloc[:, 1].mean()
    print(f"A0_mean = {A0_mean}")
    
    # Filter only positive Vt values
    rec_Vt_df_positive = rec_Vt_df[rec_Vt_df['ExTime'] >= 0]
    if rec_Vt_df_positive.empty:
        print(f"No positive Vt values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {rec_Vt_df.shape[0] - rec_Vt_df_positive.shape[0]} negative exercise time values.")
        print(f"{rec_Vt_df_positive.shape[0]} positive exercise time values.")

    # Extract time and Vt (all positive exercise values from here)
    x_data = np.array(rec_Vt_df_positive.iloc[:, 0])
    y_data = np.array(rec_Vt_df_positive.iloc[:, 1])
    ...
    for iteration in range(5):
        ...
        A1_lb = y_data.min()  #Contrains lowest Vt observed in recovery 
        td_lb = 0
        tau_lb = 0
    ...
    area_under_curve = (quad(fitted_Vt, x_start, x_end)[0]) / 60 # Perform definite integregral for the final model results and convert it to liters  
    rest_Vt_area = (A1 / 60) * x_end # dividing by 60 converts the result to L
    print(f"Rest_Vt_area = {rest_Vt_area}") 
    print(f"Area under curve = {area_under_curve}")
    EPVt = area_under_curve - rest_Vt_area
    
    # Poole & Jones 2012 estimate
    Vt_off_pc = (A0- A1)
    EPVt_estimate = (tau/60) * Vt_off_pc
    VT_MRT = TD + tau
    ...
    print(f"EPVt = {EPVt:.4f}")
    print(f"EPVt_estimate = {EPVt_estimate:.4f}")
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_Vt_off_residuals.png")
    ...
    plt.ylabel("Vt (L/min)")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_Vt_off_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "file": file_name,
        "Vt_off_A0": A0, # L/breath
        "Vt_off_A1": A1, # L/breath
        "Vt_off_PCA": Vt_off_pc, # L/breath
        "Vt_off_TD": TD, # sec
        "Vt_off_tau": tau, # sec
        "Vt_off_MRT": VT_MRT,# sec
        "Vt_off_Final_R^2":r_squared,
        "Vt_off_RMSE":rmse, # L/breath
        "Vt_off_CI_A0": ci[0], # L/breath
        "Vt_off_CI_A1": ci[1], # L/breath
        "Vt_off_CI_TD": ci[2], # sec
        "Vt_off_CI_tau": ci[3], # sec
        "Vt_off_EP_Vt": EPVt, # Liters
        "Vt_off_EP_Vt_estimate": EPVt_estimate, # Liters
        "Vt_off_TT_ss": TT_ss # sec

...
def run_kinetics(file_name, rec_VCO2_df):
    ...
    A0_mean = last_60s_ex_df['PetCO2'].mean()
    ...
    x_data = np.array(rec_VCO2_df_positive['ExTime'])
    y_data = np.array(rec_VCO2_df_positive['PetCO2'])
    ...
    for iteration in range(5):
        ...
        # Time and PetCO2 arrays
        time_vals = x_data
        PetCO2_vals = y_data
        # Create a boolean mask where PetCO2 < 95% of A0_mean
        ...
        # Search for the first time PetCO2 stays above threshold for >= 30 seconds
    ...
    area_under_curve = (quad(fitted_PetCO2, x_start, x_end)[0]) # Perform definite integregral for the final model results and convert it to ptu  
    rest_PetCO2_area = A1 * x_end # dividing by 60 converts the result to ptu
    print(f"Rest_PetCO2_area = {rest_PetCO2_area}") 
    print(f"Area under curve = {area_under_curve}")
    EPOC = area_under_curve - rest_PetCO2_area
    
    # Poole & Jones 2012 estimate
    PetCO2_off_pc = (A0- A1)
    EPOC_estimate = (tau/60) * PetCO2_off_pc
    PetCO2_MRT = TD + tau
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_PetCO2_off_residuals.png")
    ...
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_PetCO2_off_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "file": file_name,
        "PetCO2_off_A0": A0, # mmHg
        "PetCO2_off_A1": A1, # mmHg
        "PetCO2_off_PCA": PetCO2_off_pc, # mmHg
        "PetCO2_off_TD": TD, # sec
        "PetCO2_off_tau": tau, # sec
        "PetCO2_off_MRT":PetCO2_MRT, # sec
        "PetCO2_off_Final_R^2":r_squared,
        "PetCO2_off_RMSE":rmse, # mmHg
        "PetCO2_off_CI_A0": ci[0], # mmHg
        "PetCO2_off_CI_A1": ci[1], # mmHg
        "PetCO2_off_CI_TD": ci[2], # sec
        "PetCO2_off_CI_tau": ci[3], # sec
        "PetCO2_off_EP_PetCO2": EPOC, # mmHg*s
        "PetCO2_off_EP_PetCO2_estimate": EPOC_estimate,# mmHg*s
        "PetCO2_off_TT_ss": TT_ss # sec

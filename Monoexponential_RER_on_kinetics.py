...
def run_kinetics(file_name, ex_RER_df): 
    ...
    y_data = np.array(ex_RER_df_positive.iloc[:, 1])
    ...
    for iteration in range(5):
        ...
        # Prepare TD_initial guess (First breath where RER experiences a -5% change and is sustained for 15 seconds)
        upper_threshold =  A0_mean* 0.95
        # Time and RER arrays
        time_vals = x_data
        RER_vals = y_data
        # Create a boolean mask where RER > 105% of A0_mean
        ...
        # Search for the first time RER stays above threshold for >= 30 seconds
        ...
        if not found:
            print(f"No sustained -5% RER change found; fallback used")
        
        #Set lower bounds for the equation
        A0_lb = A0_init - 1e-3  # constrains the lower bound to very close to A0_mean
        A1_lb = y_data.min()  # Contrains lowest RER observed in ex 
        td_lb = 0  
        tau_lb = 0 
        ...
        td_ub = x_data.max() # Indicates no change in parameter across test
    ...
    # Define the fitted RER function for integration
    def fitted_RER(t):
        return model(t, A0, A1, TD, tau)
    # Integration bounds
    x_start = 0
    x_end = x_data.max() #final x value
    area_under_curve = (quad(fitted_RER, x_start, x_end)[0]) # units  
    rest_RER_area = A0 * x_end # (Units are ratio * s)
    print(f"Rest_RER_area = {rest_RER_area}") 
    print(f"Area under curve = {area_under_curve}")
    RER_def =  rest_RER_area - area_under_curve 
    
    # Poole & Jones 2012 estimate
    RER_on_pc = (A0 - A1)
    RER_def_estimate = (tau/60) * RER_on_pc
    RER_on_MRT = TD + tau
    ...
    modeled_RER_values = model(evaluation_times, A0, A1, TD, tau)
    ...
    # Extract Belt_speed and corresponding modeled RER values
    belt_speed_values = valid_rows['TM_Belt_speed'].values
    time_values = valid_rows['ExTime'].values
    modeled_RER = model(time_values, A0, A1, TD, tau)
    from scipy.stats import spearmanr
    rho, p_value = spearmanr(belt_speed_values, modeled_RER)
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_RER_on_residuals.png")
    plt.savefig(residuals_plot_path)
    plt.show()
    plt.close()

    # Final RER model plot
    plt.figure(figsize=(10, 6))
    plt.scatter(x_data, y_data, label="Observed RER", color='blue', s=20)
    ...
    plt.ylabel("RER")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_RER_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "RER_on_A0": A0,
        "RER_on_A1": A1,
        "RER_on_PCA": RER_on_pc,
        "RER_on_TD": TD,
        "RER_on_tau": tau,
        "RER_on_MRT": RER_on_MRT,
        "RER_on_Final_R^2":r_squared,
        "RER_on_RMSE":rmse,
        "RER_on_CI_A0": ci[0],
        "RER_on_CI_A1": ci[1],
        "RER_on_CI_TD": ci[2],
        "RER_on_CI_tau": ci[3],
        "RER_on_RER_def": RER_def,
        "RER_on_RER_def_estimate": RER_def_estimate,
        "RER_on_TT_ss": TT_ss, 
        # "RER_model_at_60s": modeled_RER_values[0],
        # "RER_model_at_120s": modeled_RER_values[1],
        # "RER_model_at_180s": modeled_RER_values[2],
        # "RER_model_at_240s": modeled_RER_values[3],
        # "RER_model_at_300s": modeled_RER_values[4],
        # "RER_model_at_360s": modeled_RER_values[5],
        # "RER_Belt_Speed_Spearman_r": rho,
        # "RER_Belt_Speed_Spearman_p_value": p_value       

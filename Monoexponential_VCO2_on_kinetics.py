...
def run_kinetics(file_name, ex_VCO2_df):
    ...
    A0_mean = last_60s_rest_df['VCO2'].mean()
    ...
    y_data = np.array(ex_VCO2_df_positive['VCO2'])
    ...
    for iteration in range(5):
        ...
        time_vals = x_data
        VCO2_vals = y_data
        # Create a boolean mask where VCO2 > 105% of A0_mean
    ...
    area_under_curve = (quad(fitted_VCO2, x_start, x_end)[0]) / 60 # Perform definite integral for the final model results and convert it to liters  
    ...
    VCO2_on_PC = (A1-A0)
    CO2_def_estimate = (tau/60) * VCO2_on_PC
    VCO2_MRT = TD + tau
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_VCO2_on_residuals.png")
    ...
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_VCO2_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "VCO2_on_A0": A0, # L/min
        "VCO2_on_A1": A1, # L/min
        "VCO2_on_PCA": VCO2_on_PC, # L/min
        "VCO2_on_TD": TD, #  sec
        "VCO2_on_tau": tau, # sec
        "VCO2_on_MRT": VCO2_MRT, # sec
        "VCO2_on_RMSE":rmse, # L/min
        "VCO2_Final_R^2":r_squared,
        "VCO2_on_CI_A0": ci[0], # L/min
        "VCO2_on_CI_A1": ci[1], # L/min
        "VCO2_on_CI_TD": ci[2], # sec
        "VCO2_on_CI_tau": ci[3], # sec
        "VCO2_on_O2_def": CO2_def, # Liters
        "VCO2_on_O2_def_estimate": CO2_def_estimate, # Liters
        "Total_VCO2_Liters":total_area,
        "VCO2_on_TT_ss": TT_ss # sec

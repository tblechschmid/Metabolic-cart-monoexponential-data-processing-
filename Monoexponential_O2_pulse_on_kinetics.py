...
def run_kinetics(file_name, ex_o2_pulse_df):
    ...
    last_60s_rest_df = ex_o2_pulse_df[(ex_o2_pulse_df.iloc[:, 0] >= -60) & (ex_o2_pulse_df.iloc[:, 0] <= 0)]
    A0_mean = last_60s_rest_df.iloc[:, 1].mean()
    print(f"A0_mean{A0_mean}")
    
    # Filter only positive o2_pulse values
    ex_o2_pulse_df_positive = ex_o2_pulse_df[ex_o2_pulse_df['ExTime'] >= 0].dropna()
    
    # If dropping NaNs leaves nothing, skip the file
    if ex_o2_pulse_df_positive.empty:
        print(f"No usable positive o2_pulse values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {ex_o2_pulse_df.shape[0] - ex_o2_pulse_df_positive.shape[0]} rows (NaNs and/or negative ExTime).")
        print(f"{ex_o2_pulse_df_positive.shape[0]} positive, non-NaN values remaining.")

    # Extract time and o2_pulse (all positive exercise values from here)
    x_data = np.array(ex_o2_pulse_df_positive.iloc[:, 0])
    y_data = np.array(ex_o2_pulse_df_positive.iloc[:, 1])
    ...
    for iteration in range(5):
        ...
        # Time and o2_pulse arrays
        time_vals = x_data
        o2_pulse_vals = y_data
        # Create a boolean mask where o2_pulse > 105% of A0_mean
        ...
        # Search for the first time o2_pulse stays above threshold for >= 30 seconds
        ...
        if not found:
            print(f"No sustained +5% o2_pulse change found; fallback to 10 s")
        
    
        #Set lower bounds for the equation
        A0_lb = A0_init - 1e-3  # constrains the lower bound to very close to A0_mean
        A1_lb = A0_mean  #Contrains o2_pulse ot resting o2_pulse
        td_lb = 0
        tau_lb = 0
        ...
        td_ub = x_data.max() # No change in O2 Pulse
    ...
    #Calculate O2_pulse deficit
    from scipy.integrate import quad
    # Define the fitted o2_pulse function for integration
    ...
    area_under_curve = (quad(fitted_o2_pulse, x_start, x_end)[0]) / 60 # Perform definite integral for the final model results and convert it to liters  
    total_area = (A1 / 60) * x_end # End result to L
    print(f"Total exercise area = {total_area}") 
    print(f"Area under curve = {area_under_curve}")
    O2_pulse_def = total_area - area_under_curve
    
    # Poole & Jones 2012 estimate
    o2_pulse_on_PC = (A1-A0)
    O2_pulse_def_estimate = (tau/60) * o2_pulse_on_PC
    o2_pulse_MRT = TD + tau
    ...
    print(f"O2 pulse deficit = {O2_pulse_def:.4f}")
    print(f"O2_pulse_def_estimate = {O2_pulse_def_estimate:.4f}")
    print(f"Time to steady state = {TT_ss:.4f}")

    # Graph to plot residuals 
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_o2_pulse_on_residuals.png")
    plt.savefig(residuals_plot_path)
    plt.show()
    plt.close()

    # Final o2_pulse model plot
    ...
    # Graph to plot the kinetics 
    plt.title(f"o2_pulse Kinetics - {file_name}")
    plt.xlabel("Time (s)")
    plt.ylabel("o2_pulse (mL/beat)")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_o2_pulse_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "file": file_name,
        "O2_pulse_on_A0": A0, # mL O2/beat
        "O2_pulse_on_A1": A1, # mL O2/beat
        "O2_pulse_on_PCA": o2_pulse_on_PC, # mL O2/beat
        "O2_pulse_on_TD": TD, # sec
        "O2_pulse_on_tau": tau, # sec
        "O2_pulse_on_MRT": o2_pulse_MRT, # sec
        "O2_pulse_Final_R^2":r_squared,
        "O2_pulse_on_RMSE":rmse, # mL O2/bea
        "O2_pulse_on_CI_A0": ci[0], # mL O2/beat
        "O2_pulse_on_CI_A1": ci[1], # mL O2/beat
        "O2_pulse_on_CI_TD": ci[2], # sec
        "O2_pulse_on_CI_tau": ci[3], # sec
        "O2_pulse_on_O2_def": O2_pulse_def, # mL O2 * min / beat
        "O2_pulse_on_O2_def_estimate": O2_pulse_def_estimate, # mL O2 * min / beat
        "O2_pulse_on_TT_ss": TT_ss #sec

...
# Main function to run kinetics model for RR
def run_kinetics(file_name, ex_VO2_df):
    ...
    A0_mean = last_60s_rest_df['RR'].mean()
    print(f"A0_mean{A0_mean}")
    
    # Filter only positive RR values
    ex_RR_df_positive = ex_VO2_df[ex_VO2_df['ExTime'] >= 0]
    if ex_RR_df_positive.empty:
        print(f"No positive RR values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {ex_VO2_df.shape[0] - ex_RR_df_positive.shape[0]} negative exercise time values.")
        print(f"{ex_RR_df_positive.shape[0]} positive exercise time values.")

    # Extract time and RR (all positive exercise values from here)
    x_data = np.array(ex_RR_df_positive['ExTime'])
    y_data = np.array(ex_RR_df_positive['RR'])
    ...
    for iteration in range(5):
        ...
        # Prepare TD_initial guess (First breath where RR experiences a +5% change and is sustained for 15 seconds)
        upper_threshold =  A0_mean* 1.10
        # Time and RR arrays
        time_vals = x_data
        RR_vals = y_data
        # Create a boolean mask where RR > 105% of A0_mean
        above_threshold = RR_vals > upper_threshold
        # Minimum sustained duration requirement
        min_duration = 15  # seconds
        # Default time delay value
        td_init = 8.9
        found = False  # Flag to check if a valid TD was found
        # Search for the first time RR stays above threshold for >= 30 seconds
    ...
    #Calculate RR deficit
    from scipy.integrate import quad
    # Define the fitted RR function for integration
    def fitted_RR(t):
        return A0 + (A1 - A0) * (1 - np.exp(-(t - TD) / tau)) if t >= TD else A0
    # Integration bounds
    x_start = 0
    x_end = x_data.max()  # final x value converted to minutes
    area_under_curve = (quad(fitted_RR, x_start, x_end)[0]) / 60 # Perform definite integral for the final model results and convert it to RR to breaths 
    total_area = (A1 / 60) * x_end # End result to breaths
    print(f"Total exercise area = {total_area}") 
    print(f"Area under curve = {area_under_curve}")
    RR_def = total_area - area_under_curve
    
    # Poole & Jones 2012 estimate
    RR_on_PC = (A1-A0)
    RR_def_estimate = (tau/60) * RR_on_PC
    RR_MRT = TD + tau
    ...
    print(f"RR deficit = {RR_def:.4f}")
    print(f"RR_def_estimate = {RR_def_estimate:.4f}")
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_RR_on_residuals.png")
    ...
    plt.scatter(x_data, y_data, label="Observed RR", color='blue', s=20)
    ...
    equation_text = f"RR(t) = {A0:.2f} + ({A1:.2f} - {A0:.2f}) × [1 - exp(-(t - {TD:.2f}) / {tau:.2f})]"
    plt.text(
    ...
    plt.title(f"RR Kinetics - {file_name}")
    plt.xlabel("Time (s)")
    plt.ylabel("RR (breaths/min)")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_RR_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "file": file_name,
        "RR_on_A0": A0, # breaths/min
        "RR_on_A1": A1, # breaths/min
        "RR_on_PCA":RR_on_PC, # breaths/min
        "RR_on_TD": TD, # sec
        "RR_on_tau": tau, # sec
        "RR_on_MRT": RR_MRT, #sec
        "RR_on_RMSE":rmse, # breaths/min
        "RR_Final_R^2":r_squared,
        "RR_on_CI_A0": ci[0], # breaths/min
        "RR_on_CI_A1": ci[1], # breaths/min
        "RR_on_CI_TD": ci[2], # sec
        "RR_on_CI_tau": ci[3], # sec
        "RR_on_RR_def": RR_def, # breaths
        "RR_on_RR_def_estimate": RR_def_estimate, # breaths
        "Total_RR_breaths":total_area,
        "RR_on_TT_ss": TT_ss # secs

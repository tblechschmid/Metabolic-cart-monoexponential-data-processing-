...
# Main function to run kinetics model for PetCO2
def run_kinetics(file_name, ex_VCO2_df):
    ...
    last_60s_rest_df = ex_VCO2_df[(ex_VCO2_df['ExTime'] >= -60) & (ex_VCO2_df['ExTime'] <= 0)]
    A0_mean = last_60s_rest_df['PetCO2'].mean()
    print(f"A0_mean{A0_mean}")
    
    # Filter only positive PetCO2 values
    ex_VCO2_df_positive = ex_VCO2_df[ex_VCO2_df['ExTime'] >= 0]
    if ex_VCO2_df_positive.empty:
        print(f"No positive PetCO2 values in {file_name}. Skipping this file.")
        return None
    else:
        print(f"Removed {ex_VCO2_df.shape[0] - ex_VCO2_df_positive.shape[0]} negative exercise time values.")
        print(f"{ex_VCO2_df_positive.shape[0]} positive exercise time values.")

    # Extract time and PetCO2 (all positive exercise values from here)
    x_data = np.array(ex_VCO2_df_positive['ExTime'])
    y_data = np.array(ex_VCO2_df_positive['PetCO2'])
    ...
    for iteration in range(5):
        ...
        # Prepare TD_initial guess (First breath where PetCO2 experiences a +5% change and is sustained for 15 seconds)
        upper_threshold =  A0_mean* 1.10
        # Time and v arrays
        time_vals = x_data
        PetCO2_vals = y_data
        # Create a boolean mask where PetCO2 > 105% of A0_mean
        above_threshold = PetCO2_vals > upper_threshold
        # Minimum sustained duration requirement
        min_duration = 15  # seconds
        # Default time delay value
        td_init = 22.9
        found = False  # Flag to check if a valid TD was found
        # Search for the first time v stays above threshold for >= 30 seconds
        ...
        if not found:
            print(f"No sustained +5% v change found; fallback to 22.6 s")
        
        #Set lower bounds for the equation
        A0_lb = A0_init - 1e-3  # constrains the lower bound to very close to A0_mean
        A1_lb = A0_mean  #Contrains PetCO2 ot resting PetCO2
        td_lb = 0  
        tau_lb = 0 
    ...
    # Define the fitted PetCO2 function for integration
    def fitted_PetCO2(t):
        return A0 + (A1 - A0) * (1 - np.exp(-(t - TD) / tau)) if t >= TD else A0
    # Integration bounds
    x_start = 0
    x_end = x_data.max()  # final x value converted to minutes
    area_under_curve = quad(fitted_PetCO2, x_start, x_end)[0] # Perform definite integral for the final model results and convert it to pressure-time units  
    total_area = A1 * x_end # End result to pressure-time units
    print(f"Total exercise area = {total_area}") 
    print(f"Area under curve = {area_under_curve}")
    CO2_def = total_area - area_under_curve
    
    # Poole & Jones 2012 estimate
    PetCO2_on_PC = (A1-A0)
    CO2_def_estimate = (tau/60) * PetCO2_on_PC
    PetCO2_on_MRT = TD + tau
    ...
    print(f"CO2 deficit = {CO2_def:.4f}")
    print(f"CO2_def_estimate = {CO2_def_estimate:.4f}")
    ...
    plt.grid(True)
    residuals_plot_path = os.path.join(plot_dir, f"{file_name}_PetCO2_on_residuals.png")
    plt.savefig(residuals_plot_path)
    plt.show()
    plt.close()

    # Final PetCO2 model plot
    plt.figure(figsize=(10, 6))
    plt.scatter(x_data, y_data, label="Observed PetCO2", color='blue', s=20)
    ...
    equation_text = f"PetCO2(t) = {A0:.2f} + ({A1:.2f} - {A0:.2f}) × [1 - exp(-(t - {TD:.2f}) / {tau:.2f})]"
    plt.text(
    ...
    plt.title(f"PetCO2 Kinetics - {file_name}")
    plt.xlabel("Time (s)")
    plt.ylabel("PetCO2 (mmHg)")
    plt.legend()
    plot_path = os.path.join(plot_dir, f"{file_name}_PetCO2_on_kinetics_plot.png")
    ...
    results_dict = {
        ...
        "file": file_name,
        "PetCO2_on_A0": A0, # mmHg
        "PetCO2_on_A1": A1, # mmHg
        "PetCO2_on_PCA": PetCO2_on_PC, # mmHg
        "PetCO2_on_TD": TD, #  sec
        "PetCO2_on_tau": tau, # sec
        "PetCO2_on_MRT": PetCO2_on_MRT, # sec
        "PetCO2_on_RMSE":rmse, # mmHg
        "PetCO2_Final_R^2":r_squared,
        "PetCO2_on_CI_A0": ci[0], # mmHg
        "PetCO2_on_CI_A1": ci[1], # mmHg
        "PetCO2_on_CI_TD": ci[2], #  sec
        "PetCO2_on_CI_tau": ci[3], #  sec
        "PetCO2_on_def": CO2_def, # pressure-time units
        "PetCO2_on_def_estimate": CO2_def_estimate, # pressure-time units
        "Total_PetCO2_pressure-time-units":total_area,
        "PetCO2_on_TT_ss": TT_ss # sec
